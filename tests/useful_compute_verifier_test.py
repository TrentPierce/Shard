...
def test_useful_compute_verifier_accepts_improbable_full_match(monkeypatch):
    ...
    monkeypatch.setattr(shard_api, "get_or_load_bitnet", fake_load)
    monkeypatch.setattr(shard_api, "_verify_draft", fake_verify)

    verifier = shard_api.UsefulComputeVerifier(top_k=8, max_tokens=32, failure_threshold=1e-6)
...
def test_useful_compute_verifier_rejects_mismatch(monkeypatch):
    ...
    verifier = shard_api.UsefulComputeVerifier(top_k=8, max_tokens=32, failure_threshold=1e-12)
