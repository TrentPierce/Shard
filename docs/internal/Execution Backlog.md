Execution Backlog (Epics, Tickets, Acceptance Criteria, Sequencing)
  ...
  - TODO-WEB-METRICS-01: `Done (2026-02-27)` Hosted dashboard token totals now read from authoritative `/metrics/summary` counters (`tokens_processed_total + tokens_offloaded_to_scouts_total`) with stale-snapshot fallback so transient backend outages do not reset totals to zero.
  - TODO-OBS-TOKENS-01: `Done (2026-02-27)` Added `ShardZeroTokenDrift` alert and runbook wiring to page when chat completions succeed while `tokens_processed_total + tokens_offloaded_to_scouts_total` remains flat for 5 minutes.
  - TODO-RUNTIME-QUALITY-01: `Done (2026-02-27)` Added verifier output degeneration guardrail for repeated-token suffix loops with runtime fallback/reset signaling and regression test coverage.
  - TODO-HEALTH-SEMANTICS-01: `Done (2026-02-27)` Web proxy routes now preserve tri-state health semantics (`ready`/`degraded`/`unavailable`) and expose degraded stale snapshots for health/peers/topology/metrics so telemetry remains truthful during transient backend failures.
  - TODO-SCOUT-METRICS-01: `Done (2026-02-27)` Scout runtime telemetry now exposes draft-capable scout counts, runtime mode split (`webgpu` vs `wasm`), and last submit-success timestamps/age in `/metrics/summary` and `/health`.
  ...
  ## 12. Remediation Execution Status (2026-02-27)

  - R0-T31: `Done` (chat completion runtime records real prompt/completion usage fields and increments `tokens_processed_total` in streaming and non-streaming paths).
  - R0-T32: `Done` (health/status endpoints expose readiness gating via `status`, `readiness_reason`, and `ready_for_inference`; nodes without loaded engines no longer report fully ready state).
  - R0-T33: `Done` (repetitive degeneration detector now guards streaming and non-stream generation paths, records `output_degeneration_detected_total`, triggers fallback signaling, and is covered by targeted regression tests).
