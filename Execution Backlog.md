Execution Backlog (Epics, Tickets, Acceptance Criteria, Sequencing)
  ...
  - TODO-NET-SCHEDULER-01: `Done (2026-02-25, updated 2026-02-26)` Scheduler now uses adaptive speculative wait timeout, acceptance-rate-aware timeout scaling, and adaptive speculative bypass when acceptance stays below threshold; scout work publish path is non-blocking to prevent queue backpressure from inflating request latency tails.
  - TODO-WEB-RECONNECT-01: `Done (2026-02-26)` App scout lifecycle was fixed to avoid tearing down worker/layer-host on topology-query rerenders, so open tabs persist contribution and auto-resume across backend restarts without manual refresh.
  ...
  - TODO-SCOUT-READY-01: Distinguish `active browser session` from `draft-capable scout` in scheduler eligibility so fallback-only scouts are not assigned speculative work they cannot submit.
  ...
  - R0-T34: `Partial` (speculative verification path was hardened so accepted draft state is preserved instead of re-evaluating prompt from scratch, and speculative counters continue to update on verified drafts; root-cause instrumentation is now added end-to-end for scout work polls/assignments, draft submission reject reasons, channel enqueue outcomes, speculative wait outcomes, verify-attempt/zero-accept outcomes, plus browser-side scout client telemetry counters for submit attempts/success/failure categories and fallback draft usage. 2026-02-25 updates: fixed null-engine scout mode crash (`c1fb106`, `9ee420f`), added fast fallback for slow browser generation, increased draft submit timeout, raised daemon scout wait via `SHARD_SCOUT_TIMEOUT_MS=15000` on EC2, and expanded benchmark proof artifacts for investor diagnostics with TTFT/inter-token latency breakdown (`--collect-latency-breakdown`), per-run error-class distributions, transport observations from `/v1/system/peers` + `/v1/system/topology`, and scout ingress/submit delta counters. 2026-02-26 updates: verifier acceptance policy now supports top-k + logit-gap matching (`SHARD_SPECULATIVE_TOP_K`, `SHARD_SPECULATIVE_LOGIT_TOLERANCE`), scouts switched to greedy draft generation, server-side echo-back garbage drafts are rejected early, prompt tokenization now preserves prompt whitespace to avoid boundary-induced first-token mismatches, browser scout lifecycle no longer tears down on topology-query rerenders (auto-reconnect stability), scheduler/timeout logic now applies acceptance-rate-aware scaling plus low-acceptance bypass (`SHARD_SPECULATIVE_BYPASS_THRESHOLD`, `SHARD_SPECULATIVE_BYPASS_MIN_SAMPLES`), mesh reconnect resilience was hardened via gossipsub mesh tuning + reconnect backoff, and gossipsub `WorkResponse` handoff now feeds `idempotent_results` + `scout_draft_mailbox` + notifier wakeups while daemon-side scout workers can generate/publish drafts and scheduler scout estimation counts connected peers. Latest live quick benchmarks (8 requests, 2 warmup, concurrency 2) reached 100% success in both distributed and standard modes, but speculative acceptance is still zero (`shard_speculative_accepted_tokens_total=0`, `shard_speculative_verify_zero_accept_total=1`), so closure still requires sustained draft-capable scout sessions that produce non-zero accepted speculative tokens during benchmark windows).
