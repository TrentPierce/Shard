Execution Backlog (Epics, Tickets, Acceptance Criteria, Sequencing)
  ...
  - R0-T38: `Partial` (web scout and proxy stability were hardened with transient-aware retries, jittered backoff, bounded failover budgets, and degraded-health snapshot fallback to prevent 5xx storms from collapsing visible scout activity; EC2 distributed latency SLO target remains open and requires further transport-side optimization).
  - R1-T39: `Partial` (`/api/health` now preserves recent activity snapshot across transient backend outages and `/api/health/system` is routed consistently; full daemon/web/dashboard tri-state (`ready`, `degraded`, `unavailable`) normalization is still pending).
  - R1-T40: `Partial` (observability now includes more truthful offload accounting via `tokens_offloaded_to_scouts_total` updates in speculative acceptance paths; alert wiring and runbook-linked SLI thresholds for scout submit degradation remain open).
