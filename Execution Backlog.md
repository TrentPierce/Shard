Execution Backlog (Epics, Tickets, Acceptance Criteria, Sequencing)
  ...
  - P0-T20: `Done` (runtime version surfaced in web health payload, web footer, and SDK runtime fields; web runtime hotfix added for hosted-mobile safety: chat now auto-falls back to non-streaming when SSE reader is unavailable, localhost daemon fallback is gated to local/Tauri contexts, and dashboard telemetry no longer defaults to simulated values when backend APIs are unreachable)
