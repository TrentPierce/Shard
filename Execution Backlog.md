Execution Backlog (Epics, Tickets, Acceptance Criteria, Sequencing)
  ...
  - R1-T40: `Partial` (dashboard token totals now consume authoritative backend counters and no longer depend on contributor-sum fallback, but alert wiring + runbook-linked thresholds for scout submit degradation and token drift are still open).
