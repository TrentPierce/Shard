...
// BitNet 2-bit (Type 36 compatibility)
// Block size QK_K = 256
// 64 bytes payload (2 bits per element)
// No scaling factor.
typedef struct {
    uint8_t qs[QK_K/4];
} block_bitnet_2b;

static void dequantize_row_bitnet(const void * vx, float * y, int64_t k) {
    const block_bitnet_2b * x = (const block_bitnet_2b *) vx;
    assert(k % QK_K == 0);
    int64_t nb = k / QK_K;
    for (int64_t i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < QK_K/4; j++) {
            uint8_t v = qs[j];
            y[i*QK_K + j*4 + 0] = (float)((int)(v & 0x03) - 1);
            y[i*QK_K + j*4 + 1] = (float)((int)((v >> 2) & 0x03) - 1);
            y[i*QK_K + j*4 + 2] = (float)((int)((v >> 4) & 0x03) - 1);
            y[i*QK_K + j*4 + 3] = (float)((int)((v >> 6) & 0x03) - 1);
        }
    }
}
...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [36] = { // GGML_TYPE_IQ4_NL_4_4 mapped to BitNet 2-bit (no scale)
        .type_name                = "bitnet_2b",
        .blck_size                = QK_K,
        .type_size                = sizeof(block_bitnet_2b),
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_bitnet,
        .from_float_ref           = NULL,
    },
    [37] = { // GGML_TYPE_IQ4_NL_4_8 likely matches IQ2_XS
        .type_name                = "iq2_xs",
        .blck_size                = QK_K,
        .type_size                = sizeof(block_iq2_xs),
        .is_quantized             = true,
    ...
    [38] = { // GGML_TYPE_IQ4_NL_8_8 likely matches IQ2_S
        .type_name                = "iq2_s",
        .blck_size                = QK_K,
        .type_size                = sizeof(block_iq2_s),
        .is_quantized             = true,
