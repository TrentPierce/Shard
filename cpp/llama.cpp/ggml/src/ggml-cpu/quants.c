...
// BitNet 2-bit ternary vec_dot (no scale, implicit scale=1.0)
// Same 2-bit encoding as TQ2_0 but without per-block scale factor
// Element ordering: 4 contiguous elements per byte (matching dequantize_row_bitnet)
void ggml_vec_dot_bitnet_2b_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
    UNUSED(bx);
    UNUSED(by);
    ...
    // block_bitnet_2b: just 64 bytes of qs per QK_K(=256) elements, no scale
    typedef struct { uint8_t qs[QK_K/4]; } block_bitnet_2b_local;

    const block_bitnet_2b_local * GGML_RESTRICT x = vx;
    const block_q8_K  * GGML_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < QK_K/4; ++j) {
            ...
            // 4 contiguous elements per byte, same order as dequantize
            sumi += y[i].qs[j*4 + 0] * ((int)( v       & 3) - 1);
            sumi += y[i].qs[j*4 + 1] * ((int)((v >> 2) & 3) - 1);
            sumi += y[i].qs[j*4 + 2] * ((int)((v >> 4) & 3) - 1);
            sumi += y[i].qs[j*4 + 3] * ((int)((v >> 6) & 3) - 1);
        }

        // No per-block scale for bitnet_2b; just use q8_K's d
        sumf += (float) sumi * y[i].d;
    }
