Production Readiness Plan (Enterprise + Maintainability)
  ...
  - R1-T39 partially complete: tri-state readiness semantics are now propagated in web proxy responses (`health_state`: `ready`/`degraded`/`unavailable`) and consumed by dashboard telemetry state, with stale snapshots preserving continuity during backend blips; explicit UI badging and state-transition integration coverage remain pending.
  - R1-T40 partially complete: dashboard total-token telemetry now sources authoritative backend counters (`tokens_processed_total + tokens_offloaded_to_scouts_total`) instead of contributor-sum estimation, but alert wiring and runbook-linked SLI thresholds remain open.
